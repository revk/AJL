...
char * __attribute__((warn_unused_result)) j_read_mem(const j_t, const char *buffer);   // Read object from string in memory (NULL terminated)
...
j_t __attribute__((warn_unused_result)) j_detach(j_t j);        // Detach from parent so a to make a top level object in itself
